...
/*-------------------------------------------------------------------------------------------------------------------------
	Replacement for removed _R.xxx
-------------------------------------------------------------------------------------------------------------------------*/
...
AddCSLuaFile("ev_MySQLconfig.lua")
include("ev_MySQLconfig.lua")
evolve.constants = {}
evolve.colors = {}
evolve.ranks = {}
evolve.privileges = {}
...
function evolve:LoadPlugins()
	...
	local plugins = file.Find( "ev_plugins/*.lua", "LUA" )
...
if ( SERVER ) then
	...
	concommand.Add( "EV_PluginFile", function( ply, com, args )
		if ( !ply:IsValid() and args[1] ) then
			...
			if ( found ) then
				MsgN( "[EV] Reloading plugin " .. evolve.plugins[found].Title .. "..." )
				...
				if ( prefix == "sh" or prefix == "cl" ) then
					...
					net.WriteTable( { Title = title, Contents = file.Read( "ev_plugins/" .. plugin, "LUA" ) } )
					net.Broadcast()
				end
			else
				MsgN( "[EV] Plugin '" .. tostring( args[1] ) .. "' not found!" )
			end
		end
	end )
else
...
function EV_func_Player:EV_IsRespected()
	return self:EV_GetRank() == "respected" or self:EV_IsAdmin()
end

function EV_func_Player:EV_IsAdmin()
...
function EV_func_Player:EV_IsOwner()
	if ( SERVER ) then
...
function EV_func_Player:EV_IsRank( rank )
...
function EV_func_Entity:EV_IsRespected() if ( !self:IsValid() ) then return true end end
...
function EV_func_Entity:UniqueID() if ( !self:IsValid() ) then return 0 end end
...
function EV_func_Player:EV_GetRank()
...
if ( SERVER ) then
	function evolve:SyncBans( ply )
		DB.Query("SELECT * FROM `evolve_bans` WHERE `banend`>"..sql.SQLStr(os.time()).." OR `banend`='0';", function(results)
			if results then
				for k,v in pairs(results) do
					local time = tonumber(v.banend) - os.time()
					if ( tonumber(v.banend) == 0 ) then time = 0 end
					SendUserMessage( "EV_BanEntry", ply, v.uid, v.nick, v.steamid, v.reason, evolve:GetProperty( v.admin, "Nick" ), time )
				end
			end
		end)
	end
	...
	function evolve:Ban( uid, length, reason, adminuid )
		...
		//DB.Query("CREATE TABLE IF NOT EXISTS `evolve_bans` (`steamid` varchar(255) NOT NULL,`ip` varchar(255),`lenght` varchar(255),`reason` varchar(255) NOT NULL,`admin` varchar(255) NOT NULL,`nick` varchar(255) NOT NULL);")
		DB.Query("DELETE FROM `evolve_bans` WHERE `steamid`="..sql.SQLStr(evolve:GetProperty( uid, "SteamID" ))..";")
		DB.Query("INSERT INTO `evolve_bans` (uid,steamid,ip,lenght,banend,reason,admin,nick) VALUES ('"..uid.."',"..sql.SQLStr(evolve:GetProperty( uid, "SteamID" ))..","..sql.SQLStr(evolve:GetProperty( uid, "IPAddress" ) or "")..",'"..math.max( 0, length ).."','"..os.time()+length.."',"..sql.SQLStr(reason)..",'"..adminuid.."',"..sql.SQLStr(evolve:GetProperty( uid, "Nick" ))..");")
	...
	function evolve:IsBanned( uid )
		DB.Query("SELECT * FROM `evolve_bans` WHERE `uid`="..sql.SQLStr(uid)..";", function(results)
			if results then
				for k,v in pairs(results[1]) do
					if k == "banend" then
						local banEnd = tonumber(v)
					end
				end
			end
		end)
